...
typedef uint32_t ioport_size_t;        ///< IO port size
#endif
...
typedef struct st_ioport_api
{
     ...
     * @param[in]  p_ctrl               Pointer to control structure.
     * @param[in]  pin                  Pin to be read.
     * @param[in]  option               Configuration interrupt condition for the pin.
     */
    fsp_err_t (* pinInterruptInput)(ioport_ctrl_t * const p_ctrl, bsp_io_port_pin_t pin, bsp_interrupt_input_t option);
     ...
     */
    fsp_err_t (* callbackSet)(ioport_ctrl_t * const p_ctrl, void (* p_callback)(void *), void * const p_context);
