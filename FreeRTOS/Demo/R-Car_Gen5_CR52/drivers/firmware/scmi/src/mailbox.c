				...
				bool tx)
{
	...
	ret = mbox_register_callback(tx_reply, scmi_mbox_cb, chan);
	if (ret < 0) {
		SCMI_LOG_ERR("failed to register tx reply cb");
		return ret;
	}

	ret = mbox_set_enabled(tx_reply, true);
	if (ret < 0) {
		SCMI_LOG_ERR("failed to enable tx reply dbell");
	}
