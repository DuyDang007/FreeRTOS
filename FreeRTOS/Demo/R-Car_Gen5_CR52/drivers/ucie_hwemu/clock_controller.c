...
/*******************************************************************************
 * DESCRIPTION   : Clock Controller initialize
 ******************************************************************************/
 /******************************************************************************
 * @file          clock_controller.c
 * - Version      : 0.01
 * @brief         Initial setting process of Clock Controller.
 * .
 *****************************************************************************/
/******************************************************************************
 * History : DD.MM.YYYY Version  Description
 *         : 21.11.2023 0.01     First Release
 *****************************************************************************/
#include <stdint.h>
#include <stdio.h>
...
uint32_t switch_clock_source_pll(uint32_t pll_num)
{
    ...
    {
        //NOTICE("PD_hier = %s (%d) is not implemented. Func: %s\r\n", get_hier_name(pd_hier), pd_hier, __func__);
...
void clock_controller_reg_write(uint32_t pd_hier, uint32_t reg_addr, uint32_t reg_val)
{
    ...
    {
        case PD_HIER_PERE:
            mem_write32(CLKPEREPKCPROT0, WRITE_KEY_CODE_EN);
            mem_write32(reg_addr, reg_val);
            mem_write32(CLKPEREPKCPROT0, WRITE_KEY_CODE_DIS);
            break;
        case PD_HIER_TOP:
            mem_write32(CLKTOPPKCPROT0, WRITE_KEY_CODE_EN);
            mem_write32(reg_addr, reg_val);
            mem_write32(CLKPEREPKCPROT0, WRITE_KEY_CODE_DIS);
            break;
        case PD_HIER_HSCS:
            ...
            break;
        case PD_HIER_SCP:
            mem_write32(CLKSCPPKCPROT0, WRITE_KEY_CODE_EN);
            mem_write32(reg_addr, reg_val);
            mem_write32(CLKSCPPKCPROT0, WRITE_KEY_CODE_DIS);
            break;
        default:
