...
void set_pll9_1(uint32_t f_Speed);
 ...
 * FUNCTION : Modify UCIE power management control registers
 * INPUT    : address   
 * OUTPUT   : none
 * RETURN   : none
*******************************************************************************/
void Ucie_pwrmngctrl_reg_modify(uint32_t address)
{
    ...
    mem_write32(address, value);          // Write back the modified value
}

/*******************************************************************************
 * NAME     : Ucie_hdma_SetRegs
 * FUNCTION : Set UCIE HDMA registers
...
void Ucie_hdma_SetRegs(uint32_t ucieCh, uint32_t dmaCh, uint32_t rw, uint64_t srcAddr, uint64_t dstAddr, uint32_t size)
{
	...
	base = 0xD8380000 + (ucieCh * 0x01000000) + (dmaCh * 0x800) + (rw * 0x400);

	// for debug
	printf(" CH BaseAddr=0x%x\n", base);

	mem_write32(base + 0x0000, 0x00000001);						// HDMA_EN_OFF

	mem_write32(base + 0x001C, size);								// HDMA_XFERSIZE_OFF
	mem_write32(base + 0x0020, (uint32_t)(srcAddr & 0xFFFFFFFF));	// HDMA_SAR_LOW_OFF
	mem_write32(base + 0x0024, (uint32_t)(srcAddr >> 32));			// HDMA_SAR_HIGH_OFF
	mem_write32(base + 0x0028, (uint32_t)(dstAddr & 0xFFFFFFFF));	// HDMA_DAR_LOW_OFF
	mem_write32(base + 0x002C, (uint32_t)(dstAddr >> 32));			// HDMA_DAR_HIGH_OFF

	if (rw) {
		mem_write32(base + 0x0034, 0x00070000);					// HDMA_CONTROL1_OFF
	} else {
		mem_write32(base + 0x0034, 0x00040000);					// HDMA_CONTROL1_OFF
	}

	mem_write32(base + 0x0038, 0x00000000);						// HDMA_FUNC_NUM_OFF :Function Number=0
	mem_write32(base + 0x003C, 0x00000000);						// HDMA_QOS_OFF :TC0 Best Effort

	mem_write32(base + 0x0030, 0x00000000);						// HDMA_WATERMARK_EN_OFF
	mem_write32(base + 0x0088, 0x00000078);						// HDMA_INT_SETUP_OFF
	...
	mem_write32(base + 0x0090, MSI_STOP_BASE);					// HDMA_MSI_STOP_LOW_OFF
	mem_write32(base + 0x0094, 0x00000000);						// HDMA_MSI_STOP_HIGH_OFF

	mem_write32(base + 0x0098, MSI_WATERMARK_BASE);				// HDMA_MSI_WATERMARK_LOW_OFF
	mem_write32(base + 0x009C, 0x00000000);						// HDMA_MSI_WATERMARK_HIGH_OFF

	mem_write32(base + 0x00A0, MSI_ABORT_BASE);					// HDMA_MSI_ABORT_LOW_OFF
	mem_write32(base + 0x00A4, 0x00000000);						// HDMA_MSI_ABORT_HIGH_OFF

	mem_write32(base + 0x00A8, MSI_DATA);						// HDMA_MSI_MSGD_OFF

	mem_write32(base + 0x0008, 0x00000000);						// HDMA_ELEM_PF_OFF
	mem_write32(base + 0x0010, 0x00000000);						// HDMA_LLP_LOW_OFF
	mem_write32(base + 0x0014, 0x00000000);						// HDMA_LLP_HIGH_OFF
	mem_write32(base + 0x0018, 0x00000000);						// HDMA_CYCLE_OFF
}

/*******************************************************************************
 * NAME     : Ucie_hdma_SetRegsLL
...
void Ucie_hdma_SetRegsLL(const struct hdma_info_ex *info)
{
	...
	base = 0xD8380000 + (info->ucieCh * 0x01000000) + (info->dmaCh * 0x800) + (info->rw * 0x400);

	// for debug
	printf(" CH BaseAddr=0x%x\n", base);

	/*** Create Linked List ***/
	mem_write32((info->llp + 0x00), 0x00000001);		// control
	mem_write32((info->llp + 0x04), info->size);		// transfer size
	mem_write32((info->llp + 0x08), (uint32_t)(info->sar & 0xFFFFFFFF));	// SAR (lower)
	mem_write32((info->llp + 0x0C), (uint32_t)(info->sar >> 32));			// SAR (upper)
	mem_write32((info->llp + 0x10), (uint32_t)(info->dar & 0xFFFFFFFF));	// DAR (lower)
	mem_write32((info->llp + 0x14), (uint32_t)(info->dar >> 32));			// DAR (upper)

	mem_write32((info->llp + 0x18), 0x00000006);		// control
	mem_write32((info->llp + 0x1C), 0x00000000);
	mem_write32((info->llp + 0x20), (uint32_t)(info->llp & 0xFFFFFFFF));	// LLP (lower)
	mem_write32((info->llp + 0x24), (uint32_t)(info->llp >> 32));			// LLP (upper)
	mem_write32((info->llp + 0x28), 0x00000000);
	mem_write32((info->llp + 0x2C), 0x00000000);
	...
	mem_write32(base + 0x003C, val);							// HDMA_QOS_OFF

	mem_write32(base + 0x001C, info->size);						// HDMA_XFERSIZE_OFF
	mem_write32(base + 0x0020, 0x00000000);						// HDMA_SAR_LOW_OFF
	mem_write32(base + 0x0024, 0x00000000);						// HDMA_SAR_HIGH_OFF
	mem_write32(base + 0x0028, 0x00000000);						// HDMA_DAR_LOW_OFF
	mem_write32(base + 0x002C, 0x00000000);						// HDMA_DAR_HIGH_OFF

	mem_write32(base + 0x0010, (uint32_t)(info->llp & 0xFFFFFFFF));		// HDMA_LLP_LOW_OFF
	mem_write32(base + 0x0014, (uint32_t)(info->llp >> 32));			// HDMA_LLP_HIGH_OFF

	mem_write32(base + 0x0034, 0x00000001);						// HDMA_CONTROL1_OFF
	...
	mem_write32(base + 0x0008, 0x00000000);						// HDMA_ELEM_PF_OFF
	mem_write32(base + 0x0018, 0x00000002);						// HDMA_CYCLE_OFF
}

/*******************************************************************************
 * NAME     : Ucie_hdma_start
...
void Ucie_hdma_start(uint32_t ucieCh, uint32_t dmaCh, uint32_t rw)
{
	...
	base = 0xD8380000 + (ucieCh * 0x01000000) + (dmaCh * 0x800) + (rw * 0x400);
	mem_write32(base + 0x0004, 0x00000001);						// HDMA_DOORBELL_OFF
}

/*******************************************************************************
 * NAME     : Ucie_hdma_waitStop
...
uint32_t Ucie_hdma_waitStop(uint32_t ucieCh, uint32_t dmaCh, uint32_t rw)
{
	...
	//timeout = 10000;
	timeout = 5000000; // ~10sec
	while (mem_read32(base + 0x0080) != 0x3) {					// HDMA_STATUS_OFF
		timeout--;
		if (timeout == 0) {
			ret = 1;
			break;
	...
	val = mem_read32(base + 0x0084);							// HDMA_INT_STATUS_OFF
	mem_write32(base + 0x008C, (val & 0x7));					// HDMA_INT_CLEAR_OFF
																//   [2] ABORT, [1] WATERMARK, [0] STOP
...
void Ucie_hdma_stop(uint32_t ucieCh, uint32_t dmaCh, uint32_t rw)
{
	...
	base = 0xD8380000 + (ucieCh * 0x01000000) + (dmaCh * 0x800) + (rw * 0x400);
	mem_write32(base + 0x0004, 0x00000002);						// HDMA_DOORBELL_OFF
	mem_write32(base + 0x0004, 0x00000000);						// HDMA_DOORBELL_OFF
	mem_write32(base + 0x0000, 0x00000000);						// HDMA_EN_OFF
}

/*******************************************************************************
 * NAME     : Ucie_dump_aturegs
 * FUNCTION : Dump ATU IN/OUTBOUND registers
 * INPUT    : ucieCh	UCIE ch (0-1)
 *          : region	OUTBOUND region (0-?)
 *          : inout		Dump Target: INBOUND(1), OUTBOUND(0)
 * OUTPUT   : none
 * RETURN   : none
*******************************************************************************/
void Ucie_dump_aturegs(uint32_t ucieCh, uint32_t region, uint32_t inout)
{
	...
	base = 0xD8300000 + (ucieCh * 0x01000000) + (region * 0x200) + (inout * 0x100);

	printf("<Dump UCIE%d %sBOUND_%d Reg>\n", ucieCh, (inout?"IN":"OUT"), region);
	for (offset = 0; offset < 0x24; offset = offset + 0x4) {
		printf(" 0x%x=0x%08x\n", (base + offset), mem_read32(base + offset));
	}
}
...
uint32_t Ucie_Setup_Pre(uint32_t ucieCh, uint32_t RC)
{
	...
	if (ucieCh == 0)
	{
		UCIE_AXI_BASE = 0xD8000000;
		UCIE_APB_BASE = 0xDC000000;
	}
	else
	{
		UCIE_AXI_BASE = 0xD9000000;
	...
	//;MmInitCtrl
	//mem_write32( UCIE_AXI_BASE + MMPL_MMINITCTRL_ADD, 0x08010000 );
	// NOTE: Not needed on EVB

	// state skip off
  ////; AcsmLtmlndex0Var1
	//mem_write32( UCIE_AXI_BASE + ACSM_ACSMLTSMINDEX0VAR1_ADD, 0x0003BF85 );
	////; AcsmLtmlndex0Var5
	//mem_write32( UCIE_AXI_BASE + ACSM_ACSMLTSMINDEX0VAR5_ADD, 0x00043FED );
	////; AcsmLtmlndex0Var13
	//mem_write32( UCIE_AXI_BASE + ACSM_ACSMLTSMINDEX0VAR13_ADD, 0x000CF816 );

	//; axi adr = 0
	mem_write32(UCIE_APB_BASE + 0xE005E8, 0x00000000);

	//; PLLBypen=1, phy_lclk_sel=1
	//mem_write32(UCIE_APB_BASE + 0xE22000, 0x00000005);
	//mem_write32(UCIE_APB_BASE + 0xE22000, 0x00000005);
	// NOTE: Above two lines are not present in EVB code
...
//;  ULT_CTRL_IMP_SB_CONFIG4
	...
	// NOTE: Not needed for 4GT/s operation
	...
	//mem_write32(UCIE_AXI_BASE + MMPL_ZCALCTRL0_ADD, 0x641F43E8);
	// NOTE: Not needed on EVB

	//;  ULT_ZCALSAMPLETIME
	//mem_write32(UCIE_AXI_BASE + MMPL_ZCALCTRL0_ADD, 0x640FA3E8);
	// NOTE: Not needed on EVB

	//;  ULT_ZCALOFFSETSAMPLETIME
	//mem_write32( UCIE_AXI_BASE + MMPL_ZCALCTRL0_ADD, 0x320FA3E8 );
	// NOTE: Not needed on EVB
	
	//;  ULT_TXZCALNOFFSET
	//mem_write32( UCIE_AXI_BASE + MMPL_ZCALCTRL1_ADD, 0x0C219800 );
	// NOTE: Not needed on EVB
	...
	//;  ULT_PLLCPINTCTRL
	// mem_write32( UCIE_AXI_BASE + MMPL_PLLCTRL0_ADD, 0x2C7C0C04 );
	//mem_write32(UCIE_AXI_BASE + MMPL_PLLCTRL0_ADD, 0x2C7C0C03); // 7'b111_1111, 7'b000_0011

	//;  ULT_PLLCPPROPCTRL
	// mem_write32( UCIE_AXI_BASE + MMPL_PLLCTRL0_ADD, 0x2C7C0604 );
	//mem_write32(UCIE_AXI_BASE + MMPL_PLLCTRL0_ADD, 0x2C7C0B03); // 7'b111_1111<<8, 7'b000_1011<<8

	//;  ULT_PLLCPINTGSCTRL
	// mem_write32( UCIE_AXI_BASE + MMPL_PLLCTRL0_ADD, 0x2C7C0604 );
	//mem_write32(UCIE_AXI_BASE + MMPL_PLLCTRL0_ADD, 0x2C7F0B03); // 7'b111_1111<<16, 7'b111_1111<<16
	...
	//;  ULT_PLLDIVSEL
	//mem_write32( UCIE_AXI_BASE + MMPL_PLLCTRL1_ADD, 0x00002BCE );

	//;  ULT_PLLV2IMODE
	//mem_write32( UCIE_AXI_BASE + MMPL_PLLCTRL1_ADD, 0x000033CE );
	...
	//;  ULT_DWDCDSETTLETIME
	//mem_write32(UCIE_AXI_BASE + DWORD_0_DWDCCCTRL1_ADD, 0x00C86432);
	// NOTE: Not needed on EVB

	//;  ULT_DWDCASETTLETIME
	//mem_write32(UCIE_AXI_BASE + DWORD_0_DWDCCCTRL1_ADD, 0x00C81532);
	// NOTE: Not needed on EVB

	//;  ULT_DWDCDSAMPLETIME
	//mem_write32(UCIE_AXI_BASE + DWORD_0_DWDCCCTRL1_ADD, 0x00141532);
	// NOTE: Not needed on EVB
	...
	//;  ULT_DWVREFVARMAX
	// mem_write32(UCIE_AXI_BASE + DWORD_0_DWVREFVAR_ADD, 0x00800001);
	...
	//mem_write32( UCIE_AXI_BASE + DWORD_1_DWDCCCTRL1_ADD, 0x00C86432 );
	// NOTE: Not needed on EVB
	
	//;  ULT_DWDCASETTLETIME_DW1
	// mem_write32( UCIE_AXI_BASE + DWORD_1_DWDCCCTRL1_ADD, 0x00C81532 );
	// NOTE: Not needed on EVB
	
	//;  ULT_DWDCDSAMPLETIME_DW1
	//mem_write32( UCIE_AXI_BASE + DWORD_1_DWDCCCTRL1_ADD, 0x00141532 );
	// NOTE: Not needed on EVB
	...
	// mem_write32( UCIE_AXI_BASE + DWORD_1_DWVREFVAR_ADD, 0x00800001 );
...
uint32_t Ucie_Setup_Start_Linkup(uint32_t ucieCh, uint32_t RC, uint32_t linkSpeed)
{
	...
	uint32_t dvsecLinkControl = ((linkSpeed & LINKSPEED_MASK) << LINKSPEED_OFFSET) | 0x00004000;
...
uint32_t Ucie_Setup_Wait_FreqChange_Req(uint32_t ucieCh, bool is_rc)
{
	...
	while ((mem_read32(UCIE_APB_BASE + 0xE1003C) & mask) != expect) {
		...
		if (timeout == 0) {
            printf("**Wait Freq change TIMEOUT**\n");
			ret = ERROR_END;
			break;
		}
	}
...
uint32_t Ucie_Setup_Wait_Linkup(uint32_t ucieCh, bool is_rc)
{
	...
	{   
        if (is_rc) {
            ...
            {
                printf("** ch%d TIMEOUT\n", ucieCh);
                printf("** note: AcsmLtsmStatus: 0x%08x\n", mem_read32(UCIE_AXI_BASE + ACSM_ACSMLTSMSTATUS_ADD));
                mem_write32(UCIE_APB_BASE + 0xE005E8, 0x00004040);
                printf("** note: ErrorLog0: 0x%08x\n", mem_read32(UCIE_AXI_BASE + 0x701080));
                printf("** note: ErrorLog1: 0x%08x\n", mem_read32(UCIE_AXI_BASE + 0x701090));
                ret = ERROR_END;
                break;
            }
        }
...
uint32_t Ucie_Setup_PCIE_Pre(uint32_t ucieCh, uint32_t RC)
{
	...
	if(RC){
	//; GEN2_CTRL_OFF
	mem_write32( UCIE_AXI_BASE+0x00080C, 0xB012080F );
	}else{
	//; GEN2_CTRL_OFF
	mem_write32( UCIE_AXI_BASE+0x00080C, 0xB010080F );
	}

	//; GEN3_RELATED_OFF
	mem_write32( UCIE_AXI_BASE+0x000890, 0x00002000 );

	//; GEN3_RELATED_OFF
	mem_write32( UCIE_AXI_BASE+0x000890, 0x0000A800 );
	*/
	...
	mem_write32( UCIE_AXI_BASE+0x000894, 0x00000C10 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000000 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x0000C900 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000001 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00008A00 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000002 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x0000A980 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000003 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00006A80 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000004 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00000C00 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000005 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00000AC5 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000006 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00000A86 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000007 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x0000A845 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000008 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00006906 );

	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000009 );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00000A08 );


	//; GEN3_EQ_PSET_INDEX_OFF
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x0000000A );

	//; GEN3_EQ_PSET_COEF_MAP__0
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00010800 );

	//; GEN3_EQ_FB_MODE_DIR_CHANGE_OFF
	mem_write32( UCIE_AXI_BASE+0x0008AC, 0x00000000 );

	if(RC){
	//; GEN3_EQ_CONTROL_OFF
	mem_write32( UCIE_AXI_BASE+0x0008A8, 0x0C018A61 );
	} else {
	//; GEN3_EQ_CONTROL_OFF
	mem_write32( UCIE_AXI_BASE+0x0008A8, 0x0D050061 );
	}

	//; GEN3_RELATED_OFF
	mem_write32( UCIE_AXI_BASE+0x000890, 0x0100A800 );

	//; GEN3_RELATED_OFF
	mem_write32( UCIE_AXI_BASE+0x000890, 0x0100A800 );

	//;
	mem_write32( UCIE_AXI_BASE+0x000890, 0x0140A800 );
	...
	*/
	...
	if(RC){
	mem_write32( UCIE_AXI_BASE+0x0008A8, 0x04008521 );
	} else {
	mem_write32( UCIE_AXI_BASE+0x0008A8, 0x04018921 );
	}

	mem_write32( UCIE_AXI_BASE+0x000890, 0x0240A800 );

	mem_write32( UCIE_AXI_BASE+0x000890, 0x0200A800 );
	...
	*/
	...
	if(RC){
	mem_write32( UCIE_AXI_BASE+0x0008A8, 0x04008421 );
	} else {
	mem_write32( UCIE_AXI_BASE+0x0008A8, 0x05009321 );
	}

	mem_write32( UCIE_AXI_BASE+0x000890, 0x0300A800 );
	...
	*/
	...
	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000001 );
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00000B04 );

	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000002 );
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00000A08 );

	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000003 );
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00004B00 );

	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000004 );
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00008A00 );

	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000005 );
	mem_write32( UCIE_AXI_BASE+0x000898, 0x0008090A );

	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000006 );
	mem_write32( UCIE_AXI_BASE+0x000898, 0x00086886 );

	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000007 );
	mem_write32( UCIE_AXI_BASE+0x000898, 0x0010088A );

	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000008 );
	mem_write32( UCIE_AXI_BASE+0x000898, 0x0010080C );

	mem_write32( UCIE_AXI_BASE+0x00089C, 0x00000009 );
	mem_write32( UCIE_AXI_BASE+0x000898, 0x0010278C );
	...
	if(RC){
	mem_write32( UCIE_AXI_BASE+0x0008A8, 0x05009221 );
	} else {
	mem_write32( UCIE_AXI_BASE+0x0008A8, 0x04048E21 );
	}
...
#ifdef RCAR_UCIE_V100
	printf("%08x\n", mem_read32(UCIE_APB_BASE + 0xE10004)); // temporal for debug
#else
	printf("%08x\n", mem_read32(UCIE_APB_BASE + 0xE10010)); // temporal for debug
#endif
	//;UCIERSTCTRL1
...
uint32_t Ucie_Setup_PCIE_Wait_LinkUp(uint32_t ucieCh)
{
	...
	uint32_t UCIE_APB_BASE;
	uint32_t val;
	uint32_t prev; // temporal for debug
	uint32_t mask, expect;
	uint32_t timeout;
	if (ucieCh == 0)
	{
	...
	*/
	...
	expect = 0x000010;
	timeout = 10000;
	prev = 0xffffffff; // temporal for debug
...
void set_pll9_1(uint32_t f_Speed)
{
    ...
    *(volatile uint32_t *)UCI1CORECKCR = 0x00000100;

    // Freq. = 1/(1+3) = 1/4 = PHY_CLK 500MHz, CLOCK = ON
    *(volatile uint32_t *)UCI1CORECKCR = 0x00000000;
}

void Ucie_PwrOn(uint8_t chn) {
    uint32_t ret1, ret2, ret3;

    printf("Powering on UCIE...\n");
    ret1 = R_StateManager_PowerOn(X5H_POWER_DOMAIN_ID_UCI);
    if(ret1) {
        printf("fail.\n");
    }
    else {
    ...
    printf("Bringing UCIE common domain from standby...\n");

    ret1 = R_StateManager_ClockOn(X5H_CLOCK_ID_MDLC_UCIE12);
    if(ret1) {
        printf("Clock On fail.\n");
    }

    ret2 = R_StateManager_ResetAssert(X5H_RESET_DOMAIN_ID_UCIE12);
    if(ret2) {
        printf("ResetAssert fail.\n");
    }
    
    ret3 = R_StateManager_ResetDeassert(X5H_RESET_DOMAIN_ID_UCIE12);
    if(ret3) {
        printf("ResetDeassert fail.\n");
    }
    ...
    if (chn == 0) {
        *(volatile uint32_t *)(0xDCE00070) = 0x00000040;
    }
    else if (chn == 1) {
        *(volatile uint32_t *)(0xDDE00070) = 0x00000040;
    }

    set_pll9_1(LINKSPEED_4GTPS);

    printf("Bringing remaining UCIE domain from standby...\n");
    
    ret1 = R_StateManager_ClockOn(X5H_CLOCK_ID_MDLC_UCIE11);
    if(ret1) {
        printf("Clock On fail.\n");
    }

    ret2 = R_StateManager_ResetAssert(X5H_RESET_DOMAIN_ID_UCIE11);
    if(ret2) {
        printf("ResetAssert fail.\n");
    }

    ret3 = R_StateManager_ResetDeassert(X5H_RESET_DOMAIN_ID_UCIE11);
    if(ret3) {
        printf("ResetDeassert fail.\n");
    }
...
void Ucie_PwrOff(uint8_t chn) {
    uint32_t ret;
    printf("Powering off UCIE...\n");
    ret = R_StateManager_PowerOff(X5H_POWER_DOMAIN_ID_UCI);
    if (ret) {
        printf("fail.\n");
    }
    else {
...
uint32_t Ucie_Setup_rc(uint8_t ch)
{
    ...
    printf(" RC Program is pending. Please press any key to resume\n");
    while (console_getc(&p_char)) {
        __asm__ volatile("nop");
    }

LABLE_RETRY:    
    printf("[Step 0] UCIe ch %d Power ON\n", ch);
    Ucie_PwrOn(ch);

    printf("[Step 1] UCIe ch %d Setup Pre\n", ch);
    Ucie_Setup_Pre(ch, MODE_RC); // UCIE as RC

	wait_time(0x8000);

    printf("[Step 2] RC UCIe ch %d Start Linkup\n", ch);
    Ucie_Setup_Start_Linkup(ch, MODE_RC, LINKSPEED_16GTPS); // UCIE1 as RC
	
    printf("[Step 3] UCIe ch %d Wait FreqChange Req\n", ch);
    ret = Ucie_Setup_Wait_FreqChange_Req(ch, true);
    if (ret) {
        goto LABLE_ERROR;
    }

    printf("[Step 4] UCIe ch %d Ack FreqChange\n", ch);
	Ucie_Setup_Ack_FreqChange(ch);

	wait_time(0x8000);
    printf("[Step 5] UCIe ch %d Wait Linkup\n", ch);
	ret = Ucie_Setup_Wait_Linkup(ch, true); // wait UCIE1
	if (ret != NORMAL_END)
	{
		goto LABLE_ERROR;
	}
	...
	// PCIE linkup
    printf("[Step 6] UCIe ch %d Setup PCIe pre\n", ch);
	Ucie_Setup_PCIE_Pre(ch, MODE_RC); // UCIE1 as RC
	wait_time(0x8000);

    printf("[Step 7] UCIe ch %d PCIe Start LinkUp\n", ch);
	Ucie_Setup_PCIE_Start_LinkUp(ch, MODE_RC); // UCIE1 as RC

	wait_time(0x8000);
    printf("[Step 8] UCIe ch %d PCIe Wait LinkUp\n", ch);
	ret = Ucie_Setup_PCIE_Wait_LinkUp(ch); // wait UCIE1
	if (ret != NORMAL_END)
	{
		goto LABLE_ERROR;
	}

	wait_time(0x8000); // Approximate delay of 20ms
    
    printf("[Step 9] UCIe ch %d PCIe Post\n", ch);
	Ucie_Setup_PCIE_Post(ch, MODE_RC); // UCIE1 as RC
	wait_time(0x8000);
...
uint32_t Ucie_Setup_ep(uint8_t ch)
{
    ...
    printf("EP Program is pending. Please press any key to resume\n");
    while (console_getc(&p_char)) {
        __asm__ volatile("nop");
    }
...
LABLE_RETRY:
    ...
    printf("[Step 1] UCIe ch %d Setup Pre\n", ch);
	Ucie_Setup_Pre(ch, MODE_EP); // UCIE1 as EP

	wait_time(0x8000);

    printf("[Step 2] EP UCIe ch %d Start Linkup\n", ch);
    Ucie_Setup_Start_Linkup(ch, MODE_EP, LINKSPEED_16GTPS); // UCIE1 as EP
	
    printf("[Step 3] UCIe ch %d Wait FreqChange Req\n", ch);
    Ucie_Setup_Wait_FreqChange_Req(ch, false);
    ...
    printf("[Step 5] UCIe ch %d Wait Linkup\n", ch);
	ret = Ucie_Setup_Wait_Linkup(ch, false); // wait UCIE1
	if (ret != NORMAL_END)
	{
		goto LABLE_ERROR;
	}
	...
	// PCIE linkup
    printf("[Step 6] UCIe ch %d Setup PCIe pre\n", ch);
	Ucie_Setup_PCIE_Pre(ch, MODE_EP); // UCIE1 as EP
	wait_time(0x8000);

    printf("[Step 7] UCIe ch %d PCIe Start LinkUp\n", ch);
	Ucie_Setup_PCIE_Start_LinkUp(ch, MODE_EP); // UCIE1 as EP
    ...
    printf("[Step 9] UCIe ch %d PCIe Post\n", ch);
	Ucie_Setup_PCIE_Post(ch, MODE_EP); // UCIE1 as EP
	wait_time(0x8000);
